...
import { ChevronDown, Search, UserCog, User, GraduationCap } from 'lucide-react';
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
        ...
        <main className="flex-1 py-8 lg:py-12">
          <div className="container mx-auto px-4">
            <PageHeader 
                title="Fitur Manajemen Pengguna" 
                description="Satu tempat untuk mengelola semua peran di lingkungan sekolah Anda." 
            />
            <div className="grid md:grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
                <InfoCard
                    icon={UserCog}
                    title="Profil Admin"
                    description="Admin memiliki kontrol penuh atas sistem, mulai dari pendaftaran pengguna baru, pengelolaan data master akademik, hingga pengaturan konfigurasi sistem."
                />
                <InfoCard
                    icon={GraduationCap}
                    title="Profil Guru"
                    description="Guru dapat dengan mudah mengelola jadwal mengajar, mencatat kehadiran siswa, memasukkan nilai, dan berinteraksi dengan siswa melalui dasbor khusus."
                />
                <InfoCard
                    icon={User}
                    title="Profil Siswa"
                    description="Siswa dapat melihat jadwal pelajaran, memeriksa nilai, memantau kehadiran, dan mengakses materi pembelajaran yang dibagikan oleh guru."
                />
            </div>
            <div className="mt-12 text-center">
                <p className="text-lg text-muted-foreground">Siap untuk mengelola sekolah Anda dengan lebih efisien?</p>
                <Button size="lg" className="mt-4" onClick={() => setIsRegisterOpen(true)}>
                    Daftar
                </Button>
            </div>
          </div>
